...
EOF
    ...
    # Fix permissions và cài ca-certificates cho Alpine
    if [[ "$DISTRO_NAME" == "Alpine Linux" ]]; then
        print_info "Đang cấu hình Alpine Linux..."
...
fix_alpine_permissions() {
    ...
    # Tạo và fix permissions cho apk directories
    ...
    # Tạo repositories file nếu chưa có
...
#!/bin/bash
# Script fix GPG keys trong proot environment

echo "[*] Fixing GPG keys and permissions..."

# Fix permissions cho keyrings (critical for _apt user)
